...
#     # hyb_set = ComplexSet(strands={hp_st: 1e-6, mmp9_st: 1e-6},complexes=SetSpec(max_size=0))
...
#     hairpin_result = hp_result[hp_complex]
#     hyb_result = hyb_result[hyb_complex]
#     # for i, s in enumerate(hyb_result.mfe):
...
#     # # Sum of rows
#     # sum_rows = np.sum(temp, axis=1)
#     # sum_cols = np.sum(temp, axis=0)
...
#     hyb_mfe.append(hyb_result.mfe[0][1])
#     temp=hyb_result.pairs.to_array()
#     #we are only interested in a+b area not a a, bb etc
#     # so only take the first n columns for len hairpin
#     # and then recalculate probabilties
#     # Select the first 'hairpin_len' columns
#     hairpin_len = len(hairpin)  # Length of the hairpin sequence
#     temp = temp[hairpin_len:, :hairpin_len]
...
#         plt.savefig(results_dir + hairpin + "_probability_matrix.png", bbox_inches="tight", pad_inches=0.1,dpi=400)
#         # plt.savefig(results_dir + hairpin + "_probability_matrix.svg", bbox_inches="tight", pad_inches=0.1)
#         # plt.savefig(results_dir + hairpin + "_probability_matrix.pdf", bbox_inches="tight", pad_inches=0.1)
#         plt.show()
#         plt.close()
