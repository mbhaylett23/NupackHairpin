...
for hairpin, complement in tqdm(zip(list_of_strings, reverse_complementary_hairpins), total=total_pairs, desc="Hairpin-Complement Pairs Processed"):
    ...
    hyb_result_hairpin = complex_analysis(hyb_set_hairpin, compute=['pfunc', 'pairs', 'mfe', 'subopt'],
    ...
    hyb_result_complement = complex_analysis(hyb_set_complement, compute=['pfunc', 'pairs', 'mfe', 'subopt'],
                                             options={'energy_gap': gap}, model=mmp9_model)
...
for pb_hair, pb_hyb in zip(hairpin_pd['Hairpin_Pb'], hairpin_pd['Hyb_Pb']):
    ...
    temp=np.nanmean(np.diag(innerV))      
    hp_pb_meanDIAG.append(temp)
...
for seqSTR in seqIDX:
    ...
    temp.sort_values(by='D_Pb_Hairpin_mean_DIAG', ascending=False).reset_index(drop=True)
...
hairpin_pd = hairpin_pd.sort_values(by='D_Pb_Hyb_Weighted_Mean', ascending=False).reset_index(drop=True)
...
hairpin_pd_top['Rank'] = hairpin_pd_top['Combined_Score'].rank(ascending=False, method='dense').astype(int)
...
grouped_top_50 = hairpin_pd_top_sorted.groupby('Name').apply(lambda x: x.nlargest(50, 'Combined_Score')).reset_index(drop=True)
